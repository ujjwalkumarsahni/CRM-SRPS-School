...
const completeProfile = async (req, res) => {
  try {
    const { dob, bloodGroup, mobileNumber, address, highestQualification } =
      req.body;
    ...
    res.json({
      success: true,
      message:
        "Profile completed successfully. Waiting for admin verification.",
      data: profile,
    });
  } catch (error) {
...
const updateProfile = async (req, res) => {
  try {
    ...
    if (highestQualification)
      updateData.highestQualification = highestQualification;
    ...
    if (req.files) {
      ...
      if (req.files.aadharCard) {
        if (profile?.documents?.aadharCard?.publicId) {
          await cloudinary.uploader.destroy(
            profile.documents.aadharCard.publicId,
          );
        }
      ...
      if (req.files.qualificationCertificate) {
        if (profile?.documents?.qualificationCertificate?.publicId) {
          await cloudinary.uploader.destroy(
            profile.documents.qualificationCertificate.publicId,
          );
        }
...
const createLeaveRequest = async (req, res) => {
  try {
    ...
    if (!req.user.isVerified) {
      return res
        .status(403)
        .json({ error: "Account not verified. Cannot apply for leave." });
    }
...
const getMyLeaves = async (req, res) => {
  try {
    const leaves = await LeaveRequest.find({ teacher: req.user._id }).sort({
      createdAt: -1,
    });
...
const cancelLeaveRequest = async (req, res) => {
  try {
    ...
    if (new Date(leave.startDate) < today) {
      return res.status(400).json({ 
        error: "Cannot cancel leave request for past dates" 
      });
    }
