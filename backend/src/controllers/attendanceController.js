...
//     if (!attendance) {
//       // Create new attendance record with in-time only, status remains 'absent' for now
...
//       // Status remains whatever it was (absent by default)
...
// // Teacher: Mark Out-Time with Location (Calculate final status here)
...
//     // Validate work report is provided
...
//         .json({
//           error:
//             "No attendance record found for today. Please mark in-time first.",
//         });
//     }
...
//           (daysInMonth - calendarData.filter((d) => d.isSunday).length)) *
//         100
//       ).toFixed(1),
//     };
...
//       halfDayCount: attendances.filter((a) => a.status === "half-day").length,
...
// // Admin: Get all pending attendance update requests
...
// // Admin: Get all update requests (with filters)
...
// // Admin: Approve attendance update request
...
//     const request =
//       await AttendanceUpdateRequest.findById(requestId).populate("teacher");
//     if (!request) {
...
//       `
//       <div class="text-left">
//         <p>Dear ${request.teacher.name},</p>
//         <p>Your attendance update request has been <strong>approved</strong>.</p>
//         <p><strong>Date:</strong> ${new Date(attendance.date).toLocaleDateString()}</p>
//         <p><strong>Updated Status:</strong> ${attendance.status}</p>
...
//     res.json({
//       success: true,
//       message: "Request approved and attendance updated successfully",
//       data: request,
//     });
//   } catch (error) {
//     res.status(500).json({ error: error.message });
...
// // Admin: Reject attendance update request
...
//       <div class="text-left">
//         <p>Dear ${request.teacher.name},</p>
//         <p>Your attendance update request has been <strong>rejected</strong>.</p>
//         <p><strong>Date:</strong> ${new Date(request.attendance.date).toLocaleDateString()}</p>
//         <p><strong>Reason for rejection:</strong> ${adminRemarks || "Please contact admin for more details"}</p>
//       </div>
//       `,
//     );
...
//         date: targetDate,
//         status: "holiday",
//         workReport: "Marked as holiday by admin",
//       });
//     } else {
//       attendance.status = "holiday";
//       attendance.workReport = "Marked as holiday by admin";
//       await attendance.save();
//     }
